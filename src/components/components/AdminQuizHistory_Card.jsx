...
const QuizHistoryCard = ({ quiz, onViewScores }) => {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3.5">
        ...
        <div className="flex items-center justify-between text-[11px]">
          {typeof quiz.attempts === "number" && (
            <span
              className={`font-medium ${colorMode ? "text-slate-400" : "text-slate-600"}`}
            >
      ...
      <div
        className={`pt-3 border-t space-y-3 ${colorMode ? "border-slate-700/60" : "border-slate-100"}`}
      >
        {/* Action Buttons */}
        <div className="flex items-center justify-between gap-2 pt-0.5">
          ...
          <div className="flex flex-col">
            <span
              className={`text-[10px] uppercase font-semibold ${colorMode ? "text-slate-600" : "text-slate-400"}`}
            >
              Created on
            </span>
            <span
              className={`text-xs font-medium ${colorMode ? "text-slate-300" : "text-slate-600"}`}
            >
